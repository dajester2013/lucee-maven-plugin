...
public class LarCopySourcesMojo extends AbstractLarMojo {
	...
	/**
	 * Whether or not to apply filtering to the {@link LarCopySourcesMojo#larResourcesDir}
	 */
	@Parameter(defaultValue = "true")
	...
	private File larSourceDir;

	@Parameter(defaultValue = "src/main/lar-resources")
	private File larResourcesDir;
	...
	public List<Resource> getResources() {
		...
		if (larSourceDir.exists()) {
			r = new Resource();
			r.setDirectory(larSourceDir.getAbsolutePath());
		...
		if (larResourcesDir.exists()) {
			r = new Resource();
			r.setDirectory(larResourcesDir.getAbsolutePath());
